...
export function* fetchCategInitSaga(action) {
    try {
        ...
        const category = yield axios.post('/header', {categ: 'qchat'},{headers: {'data-categ':'category'}});
...
export function* fetchQchatInitSaga(action) {
    try {
        yield put(actions.fetchQchatStart());
        const qchat = yield axios.post('/qchat', {id: action.id},{headers: {'data-categ':'editform'}});
        yield put(actions.fetchQchat(qchat.data));
    } catch(err){
        yield put(actions.fetchQchatFail(err));
    }
