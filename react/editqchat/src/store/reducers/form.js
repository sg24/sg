...
import { updateObject, clearAllData } from '../../shared/utility';
...
const fetchQchatStart = (state, action) => {
    return updateObject(state, {qchatErr: null})
};

const fetchQchatFail = (state, action) => {
    return updateObject(state, {qchatErr: action.err})
};

const fetchQchat = (state, action) => {
    if (!action.qchat) {
        return updateObject(state, {redirect: true})
    }

    localStorage.setItem('question', JSON.stringify(action.qchat))
    return updateObject(state, {update: true})
};
...
const reducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case actionTypes.FETCH_QCHAT_START:
            return fetchQchatStart(state, action);
        case actionTypes.FETCH_QCHAT_FAIL:
            return fetchQchatFail(state, action);
        case actionTypes.FETCH_QCHAT:
            return fetchQchat(state, action);
