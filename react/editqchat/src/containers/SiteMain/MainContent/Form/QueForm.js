...
class QueForm extends  Component {
    ...
    componentDidMount() {
        ...
        if (!this.state.qchat.length > 0 || !this.state.qchat.filter(cnt => cnt.position === 0)[0]) {
            this.props.history.push(`/edit/qchat/?id=0`)
        }
    ...
    componentDidUpdate() {
        ...
        if ((this.state.oldQueNum !== this.state.queNum)) {
            this.props.history.push('/edit/qchat/?id='+this.state.queNum)   
            this.setState({
        ...
        if (this.state.qchat && this.state.qchat.length > 0 && this.state.qchat.filter(cnt => cnt.position === this.state.queNum)[0] && this.state.queNum !== 0 &&!this.state.update) {
            ...
            if ('indexedDB' in window) {
                readData('media', this.state.queNum).then(media => {
                    ...
                    if (cnt.image || cnt.video) {
                        for (let itm of cnt.image) {
                            image.push({id: itm.id, url: `https://www.slodge24.com/media/image/${itm.id}`})
                        }
                        for (let itm of cnt.video) {
                            video.push({id: itm.id, url: `https://www.slodge24.com/media/video/${itm.id}`})
                        }
                    }
    ...
    navHandler = () => {
        this.props.history.push(`/edit/qchat/?id=${this.state.editInput}`);
    ...
    submitHandler = (mode) => {
     ...
     goBackHandler = () => {
        ...
        if (back > -1) {
            this.props.history.push(`/edit/qchat/?id=${back}`);
     ...
     addCntHandler = () => {
         if (this.state.formIsValid || !this.state.answer.length < 1) {
            ...
            if (qchat.length > 0) {
                ...
                let checkMedia = cnt.filter(mediaCnt => mediaCnt.position === this.state.queNum)[0];
                if (checkMedia) {
                    media = cnt.filter(mediaCnt => mediaCnt.position === this.state.queNum)[0];
                }
