...
import { updateObject, checkValidity, readData, clearAllData } from '../../../../shared/utility';
...
class Form extends  Component {
    ...
    componentDidMount() {
        ...
        if (this.state.qchat.length < 1) {
            this.props.onFetchQchat(localStorage.getItem('id'));
        }
        ...
        if ('localStorage' in window) {
            ...
            this.setState({indexDBErr: 'Please update your browser'})
        }
    ...
    componentDidUpdate() {
        ...
        if (this.props.update && !this.state.update) {
            ...
            if (qchat) {
                if (JSON.stringify(this.state.qchat) !== qchat) {
                    this.setState({update: true, qchat: JSON.parse(qchat)})
                }
            }
        }
        this.updateCnt();
    ...
    updateCnt = () => {
        if (this.state.qchat && this.state.qchat.length > 0 && this.state.qchat.filter(cnt => cnt.position === 0)[0] && !this.state.updateCnt) {
            ...
            if (qchat.hour || qchat.minute || qchat.hour) {
                let hour = {...oldSetTime.hour};
                hour.value = qchat.hour ? qchat.hour : 0;
                let minute = {...oldSetTime.minute};
                minute.value = qchat.minute ? qchat.minute : 0;
                let second = {...oldSetTime.second};
                second.value = qchat.second ? qchat.second : 0;
            ...
            if ('indexedDB' in window) { 
                readData('media', 0).then(media => {
                    ...
                    if (qchat.image || qchat.video) {
                        for (let cnt of qchat.image) {
                            image.push({id: cnt.id, url: `https://www.slodge24.com/media/image/${cnt.id}`})
                        }
                        for (let cnt of qchat.video) {
                            video.push({id: cnt.id, url: `https://www.slodge24.com/media/video/${cnt.id}`})
                        }
                    }
    ...
    submitHandler = (mode) => {
        ...
        if (this.state.formIsValid && this.state.setTimeValid) {
            ...
            let media = cnt.filter(mediaCnt => mediaCnt.position === 0)[0];
             ...
             if (mode === 'next') {
                this.props.history.push('/edit/qchat/?id=1')
             } else {
...
const mapStateToProps = state => {
    return {
        ...
        qchatErr: state.form.qchatErr,
        update: state.form.update,
        id: state.form.id
    };
};
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        onFetchQchat: (id) => dispatch(actions.fetchQchatInit(id)),
