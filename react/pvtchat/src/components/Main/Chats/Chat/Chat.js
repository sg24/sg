...
const chats = (props) => {
    ...
    let audioCnt = (
        <audio controls src={`http://localhost:3002/media/audio/${props.cnt.msg}.${props.cnt.format}`}>
            <p>Your browser does not support this feature</p>
        </audio>
    );
    let videoCnt = (
        <video controls src={`http://localhost:3002/media/video/${props.cnt.msg}.${props.cnt.format}`}>
            <p>Your browser does not support this feature</p>
        </video>
    );

    let imageCnt =(
        <img src={`http://localhost:3002/media/image/${props.cnt.msg}.${props.cnt.format}`} alt=""/>
    )
        ...
        docContent = (
            <>
                <FileIcon 
                    ...
                    size={100}/>
                <a 
                    href={`http://localhost:3002/media/${props.cnt.cntType}/${props.cnt.msg}.${props.cnt.format}`}  
                    downloads="true"
