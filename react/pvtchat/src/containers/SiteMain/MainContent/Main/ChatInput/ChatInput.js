...
class ChatInput extends Component {
    ...
    createChatHandler = () => {
        if (this.props.editChat) {
            this.props.onCreateChatInit(this.state.id, this.state.categ,'typedPlain', {editMsg: this.state.chat, mainID: this.props.editChat.mainID, chatID: this.props.editChat.chatID}, uuid(), 'pvtcreateChat')
        } else {
                ...
                axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${fetchedLocation.lat},${fetchedLocation.lng}&key=AIzaSyD2OUDjZ-urf2MAVcxPc03HA4X8KbB-jlk`).then(loc => {
                    if (!loc.data.error_message) {
                        if (these.props.editChat) {
                            these.props.onCreateChatInit(these.state.id, these.state.categ,'typedPlain', {editMsg: `In ${loc.data.results[0].formatted_address}`, mainID: these.props.editChat.mainID, chatID: these.props.editChat.chatID}, uuid(), 'pvtcreateChat')
                        } else {
