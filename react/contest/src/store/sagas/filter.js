...
export function* fetchTotalInitSaga(action) {
    try {
        let response = yield axios.post('/header', {model: 'contest'},{headers: {'data-categ':'myModel'}});
...
export function* filterContentInitSaga(action) {
    ...
    if(!action.content.apply) {
        try {
            ...
            let response = yield axios.post('/header',{filterCnt, model: 'contest'},{headers: {'data-categ':'cntSearch'}});
