...
export function* fetchCntCategInitSaga(action) {
    try {
        if (action.categ && action.categ.length > 0) {
            ...
            let response = yield axios.post('/header', {categ: 'advert'}, {headers: {'data-categ':'category'}});
...
export function* fetchTotalInitSaga(action) {
    try {
        let response = yield axios.post('/header', {model: 'advert'},{headers: {'data-categ':'myModel'}});
...
export function* filterContentInitSaga(action) {
    ...
    if(!action.content.apply) {
        try {
            ...
            let response = yield axios.post('/header',{filterCnt, model: 'advert'},{headers: {'data-categ':'cntSearch'}});
