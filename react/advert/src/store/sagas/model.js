...
export function* fetchCntInitSaga(action) {
    ...
    try {
        if (action.cntTotal === 0 || action.cntTotal > action.skipCnt) {
            let response = yield axios.post('/advert', null, {
                headers: {
...
export function* changeCntInitSaga(action) {
    ...
    try {
        if (action.det === 'delete') {
            let payload = JSON.stringify({id: action.id, model: 'advert'})
            yield axios.delete('/header', {headers: {'data-categ': `deletecnt-${payload}`}});
        } else if (action.det === 'draft' || action.det === 'acc-draft'){
            yield axios.patch('/header', {id: action.id, model: 'advert'} ,{headers: {'data-categ': 'draftmode'}});
        } else {
            yield axios.patch('/header', {id: action.id, model: 'advert'} ,{headers: {'data-categ': 'publishmode'}});
        }
