...
class Questions extends Component {
    ...
    componentDidUpdate() {
        ...
        if (this.props.startSearch && this.props.filterDet && this.props.location.search && this.state.filterTag !== `filter==${this.props.filterDet}`) {
            ...
            this.props.onFetchCnt(this.props.userID, 'question-'+cnt, this.state.fetchLimit, 0, 0);
            this.setState({
                filterTag: cnt
            });
        }
        if (!this.props.startSearch && !this.props.match.params.id && this.state.filterTag !== 'question') {
            this.props.onFetchCntReset();
    ...
    onScroll = () => {
        if ((window.innerHeight + Math.ceil(window.pageYOffset + 1)) >= document.body.offsetHeight) {
            this.props.onFetchCnt(
                    ...
                    this.state.filterTag === 'filter' ?  'filter=='+this.props.filterDet : `question-${this.state.filterTag}` : 'question' ,
