...
class Auth extends Component {
    ...
    loginHandler = () => {
        if (this.state.formIsValid) {
            this.setState({start: true, err: null})
             ...
             axios.post('http://localhost:3002/login', cnt).then(() => {
    ...
    uploadCnt = () => {
        ...
        let cnt = this.props.cnt;
        formContent.append('location', cnt.loc);
        // let cur = 0;
        for (let media in cnt.media) {
            let ext = media.file.type.split('/').pop();
            formContent.append(`media`, media.file, `${media.id}.${ext}`);
        }

        axios.post('http://localhost:3002/aroundme', formContent, {
            onUploadProgress: function (progressEvent) {
                if (progressEvent.lengthComputable) {
                    const percentage = Math.round((progressEvent.loaded * 100) / progressEvent.total);
                    this.setState({upload: percentage})
                }
            }  
        }).then((res) => {
            if (res.data) {
    ...
    render() {
                ...
                <div className="aroundme__form--cnt">
                    <div className="aroundme__form--cnt--wrapper">
                        ...
                        { !this.state.formElement.username.valid && this.state.formElement.username.touched ?
                            <div className="aroundme__form--err">Username must not be empty</div>
                            : null
                        }
                    </div>
                </div>
        ...
        if (upload > 0) {
            upload = (
                <div className="aroundme__form--auth__upload"></div>
            )
        }
