         ...
         modalContent = props.uploadErr ?  (
            <Aux>
                <h3 className="reuse-form__modal--err"> { 
                    props.uploadErr.code === 'ECONNABORTED' && props.uploadErr.message ? 'Network Error' :
                    !props.uploadErr.code ? 'Page Name already taken' :  'Network Error' } </h3>
                {props.uploadErr && !props.type ? <div className="reuse-form__btn">
                    <button 
                        type="button"  
                        className="reuse-form__btn--close reuse-form__btn--close-btn"
