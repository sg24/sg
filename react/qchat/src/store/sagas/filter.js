...
export function* fetchCntCategInitSaga(action) {
    try {
        if (action.categ && action.categ.length > 0) {
            ...
            let response = yield axios.post('/header', {categ: 'qchat'}, {headers: {'data-categ':'category'}});
...
export function* fetchTotalInitSaga(action) {
    try {
        let response = yield axios.post('/header', {model: 'qchat'},{headers: {'data-categ':'myModel'}});
...
export function* filterContentInitSaga(action) {
    ...
    if(!action.content.apply) {
        try {
            ...
            let response = yield axios.post('/header',{filterCnt, model: 'qchat'},{headers: {'data-categ':'cntSearch'}});
