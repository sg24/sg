...
class MyDocument extends next_document__WEBPACK_IMPORTED_MODULE_1___default.a {
  static getInitialProps(ctx) {
    if (ctx.req && ctx.req.useragent && !ctx.req.useragent.isBot) {
      if (ctx.req.useragent.source !== 'facebookexternalhit/1.1' || ctx.req.useragent.source !== 'Facebot' || ctx.req.useragent.source !== 'Twitterbot') {
        let url = ctx.req.originalUrl.split('robotonly').pop().split('rb').splice(0).join('');
        let updateUrl = url === ('/index' || false) ? '/index/post' : url;
