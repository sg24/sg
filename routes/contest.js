...
const {tempFile, contest, user, authUser, connectStatus} = require('../serverDB/serverDB');
...
router.post('/', authenticate, (req, res, next) => {
    if (req.header !== null && req.header('data-categ') === 'userDet') {
        contest.findById(req.body.id).then(userDet => {
            if (userDet) {
    ...
    if (req.header !== null && req.header('data-categ') === 'deletecontest') {
        contest.findOneAndRemove({authorID: req.user,_id: req.body.id}).then(result => {
            if (!result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'mycontest') {
    ...
    if (req.header !== null && req.header('data-categ') === 'chat') {
        let id = req.body.id
        contest.findByIdAndUpdate(id, {$inc: {'view': 1}}).then(chatFnd => {
            if (chatFnd) {
    ...
    if(req.header !== null && req.header('data-categ') === 'pvtcreateChat'){
        ...
        if (cnt.msg && !cnt.msg.editMsg) {
            ...
            let msg = cnt.msg
            contest.findById(id).then(chatDet => {
    ...
    function fetchContest(conditions, meta) {
        ...
        connectStatus.then(() => {
            ...
            contest.find(condition, isMeta).countDocuments({}).then((cntTotal) => {
                contest.find(condition, isMeta).sort(sort).limit(curLimit).skip(skip).then(result => {
                    let cntArray= [];
                    let send = 0;
                    if (result.length < 1) {
