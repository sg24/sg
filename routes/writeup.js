...
router.post('/', authenticate, (req, res, next) => {
    ...
    if (req.header !== null && req.header('data-categ') === 'getWriteUpFavorite') {
        writeup.find({_isCompleted: true, block: {$nin: [req.user]}, favorite: {$in: [req.user]}})
            .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
            let updateResult = [];
            if (result) {
                for (let cnt of result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'getByAuthor') {
       Promise.all([user.findById(req.body.searchCnt)]).then(doc => {
            writeup.find({authorID: req.body.searchCnt, 'shareInfo.authorID': null, _isCompleted: true, block: {$nin: [req.user]}})
                .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
                let updateResult = [];
                if (result) {
                    for (let cnt of result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'hashSearch') {
        writeup.find({hashTag: {$in: [req.body.searchCnt]}, 'shareInfo.authorID': null, _isCompleted: true, block: {$nin: [req.user]}})
            .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
            let updateResult = [];
            if (result) {
                for (let cnt of result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'setFavorite') {
        writeup.findOneAndUpdate({_id: req.body.pageID, favorite: {$nin: [req.user]}}, {$push: {'favorite': req.user}}).then(doc => {
            if (doc) {
                return res.status(200).send({pageInfo: {_id: req.body.pageID, isFavored: true, favorite: doc.favorite.length + 1}});
            }
            writeup.findOneAndUpdate({_id: req.body.pageID, favorite: {$in: [req.user]}}, {$pull: {'favorite': req.user}}).then(result => {
    ...
    if (req.header !== null && req.header('data-categ') === 'setShare') {
        ...
        writeup.findOneAndUpdate({_id: req.body.pageID}, {$addToSet: {'share': reciepent}}).then(() => {
            writeup.findById(req.body.pageID).then(doc => {
                if (doc) {
                    sharecontent(writeup, writeup, req.body.pageID, reciepent, req.user, req.username, req.userImage, 
                        ...
                        for (let cnt of shareInfo) {
                            notifications('writeupShare', cnt.userID, {userID: req.user, ID: cnt.pageID}, false);
                        }
                    });
                    return
                }
                    ...
                    group.findOne({_id: groupID, member: {$in: [req.user]}}).then(groupDoc => {
                        if (groupDoc) {
                            ...
                            if (checked === reciepent.length) {
                                sharecontent(writeup, groupwriteup, req.body.pageID, reciepent, req.user, req.username, req.userImage,
    ...
    if (req.header && (req.header('data-categ') === 'searchWriteUp' || req.header('data-categ') === 'searchWriteUpTab')) {
        writeup.find({_isCompleted: true, block: {$nin: [req.user]}, $text: {$search: req.body.searchCnt}})
        .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
            let updateResult = [];
            if (result) {
                for (let cnt of result) {
            ...
            res.status(200).send({page: updateResult, loadMore: result.length > 0, tabPage: req.header('data-categ') === 'searchWriteUpTab'});
        }).catch(err => {
            res.status(500).send(err)
        })
        return
    ...
    if (req.header !== null && req.header('data-categ') === 'getOneAndDelete') {
        writeup.findOne({_id: req.body.pageID}).then(doc => {
            ...
            if (doc && (JSON.parse(JSON.stringify(doc.authorID)) !== JSON.parse(JSON.stringify(req.user)))) {
                writeup.findByIdAndUpdate({_id: req.body.pageID}, {$push: {'block': req.user}, $pull: {'favorite': req.user}}).then(() => {
                    return res.sendStatus(200);
                })
                return;
            }
            return Promise.reject('This writeup could not be deleted');
        }).catch(err => {
            res.status(500).send(err)
        })
        return
