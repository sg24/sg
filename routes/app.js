            ...
            authUser.findById(req.user).then(result => {
                if (!result) {
                    user.findById(req.user).then(result => {
                        if (result) {
                            res.status(200).send({url: result.image, name: result.username, id: result._id})
                            return
                        }
                        res.sendStatus(200);
                    })
...
router.get('/forget/password', (req, res, next) => {
    global.app.render(req, res, '/forget/password', req.params)
});
    ...
    if (req.params) {
        jwt.verify(req.params.token, process.env.JWT_SECRET, function(err, token) {
            if(!err) {
                ...
                global.app.render(req, res, '/forget/reset', req.params)
                return
            } else {
