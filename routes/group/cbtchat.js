...
const {qchat:groupcbt, cbtchat:groupcbtchat, tempFile, connectStatus} = require('../../serverDB/serverDB');
...
router.post('/', authenticate, (req, res, next) => {
        ...
        groupcbt.findById(req.body.pageID).then(doc => {
            if (doc) {
                if (doc && doc.chat && doc.chat._id) {
                    groupcbtchat.findOne({_id: objectID(doc.chat._id)}).then(result => {
                        let total = 0;
                        if (result) {
                            let chat = result.chat.filter(cnt => cnt.replyChat === false);
                            total = chat.length;
                        }
                        groupcbtchat.aggregate([{
    ...
    if (req.header !== null && req.header('data-categ') === 'getReply') {
        groupcbtchat.findById(req.body.cntID).then(doc => {
            if (doc) {
    ...
    if (req.header !== null && req.header('data-categ') === 'sendChat') {
        formInit(req, formidable).then(form => {
            ...
            savetemp(mediaList, 'groupcbtchat', req.user).then(tempFileID => {
                uploadToBucket(mediaList, fields.description).then(media => {
                    ...
                    Promise.all([groupcbtchat.findByIdAndUpdate(fields.cntID, {$push: {chat: cnt}}),
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteChat') {
        Promise.all([deleteMedia(JSON.parse(req.body.media)),
            groupcbtchat.findByIdAndUpdate(req.body.chatID, {$pull: {chat: {'_id': req.body.cntID}}})]).then(cnt => {
            groupcbt.findByIdAndUpdate({_id: req.body.pageID}, {'chat.total': cnt[1].chat.length - 1}).then(result => {
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteReply') {
        deleteMedia(JSON.parse(req.body.media)).then(() => {
            groupcbtchat.findOne({_id: req.body.chatID}).then(doc => {
                if (doc) {
                    let chat = doc.chat;
                    ...
                    if (removeChatItem) {
                        ...
                        if (chatItem) {
                            ...
                            doc.updateOne({chat: removeChat}).then(() => {
                                groupcbt.findByIdAndUpdate({_id: req.body.pageID}, {'chat.total': removeChat.length}).then(result => {
