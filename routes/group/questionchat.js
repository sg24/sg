...
router.post('/', authenticate, (req, res, next) => {
        ...
        groupquestion.findById(req.body.pageID).then(doc => {
            if (doc) {
                if (doc && doc.chat && doc.chat._id) {
                    groupquestionchat.findOne({_id: objectID(doc.chat._id)}).then(result => {
                        let total = 0;
                        if (result) {
                            let chat = result.chat.filter(cnt => cnt.replyChat === false);
                            total = chat.length;
                        }
                        groupquestionchat.aggregate([{
                    ...
                    let newDoc = new groupquestionchat({
                        chat: []
                    });
    ...
    if (req.header !== null && req.header('data-categ') === 'getReply') {
        groupquestionchat.findById(req.body.cntID).then(doc => {
            if (doc) {
    ...
    if (req.header !== null && req.header('data-categ') === 'sendChat') {
        formInit(req, formidable).then(form => {
            ...
            savetemp(mediaList, 'groupquestionchat', req.user).then(tempFileID => {
                uploadToBucket(mediaList, fields.description).then(media => {
                    ...
                    Promise.all([groupquestionchat.findByIdAndUpdate(fields.cntID, {$push: {chat: cnt}}),
                        tempFile.findOneAndUpdate({userID: cnt.authorID, "tempFiles.id": tempFileID}, {$pull: {tempFiles: {id: tempFileID}}}),
                        groupquestion.findOneAndUpdate({_id: fields.pageID, 'chat.user.authorID': {$ne: req.user}}, {$push: {'chat.user': {authorID: req.user, username: req.username, userImage: req.userImage}}})]).then(cnt => {
                        let total = cnt[0].chat.length + 1;
                        groupquestion.findByIdAndUpdate(fields.pageID, {'chat.total': total}).then(result => {
                        ...
                        groupquestionchat.findById(fields.cntID).then(doc => {
                            if (doc) {
                                let chat = doc.chat
                        ...
                        if (doc ){
                            ...
                            if (chatItem) {
                                ...
                                Promise.all([doc.updateOne({chat}),
                                    tempFile.findOneAndUpdate({userID: cnt.authorID, "tempFiles.id": tempFileID}, {$pull: {tempFiles: {id: tempFileID}}}),
                                    groupquestion.findOneAndUpdate({_id: fields.pageID, 'chat.user.authorID': {$ne: req.user}}, {$push: {'chat.user': {authorID: req.user, username: req.username, userImage: req.userImage}}})]).then(() => {
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteChat') {
        Promise.all([deleteMedia(JSON.parse(req.body.media)),
            groupquestionchat.findByIdAndUpdate(req.body.chatID, {$pull: {chat: {'_id': req.body.cntID}}})]).then(result => {
            groupquestion.findById(req.body.pageID).then(doc => {
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteReply') {
        deleteMedia(JSON.parse(req.body.media)).then(() => {
            groupquestionchat.findOne({_id: req.body.chatID}).then(doc => {
                if (doc) {
                    let chat = doc.chat;
