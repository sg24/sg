...
router.post('/', authenticate, (req, res, next) => {
    if (req.header !== null && req.header('data-categ') === 'getonegroupfeed') {
        groupfeed.findOne({_id: req.body.cntID, authorID: req.user}).then(result => {
            if (result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'getByAuthor') {
        groupfeed.find({authorID: { $in: [req.user, ...req.friend] }, _isCompleted: true, block: {$nin: [req.user]}})
            .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
            let updateResult = [];
            if (result) {
                for (let cnt of result) {
                ...
                group.findOne({'member.authorID': {$eq: req.user}, _id: groupfeedDoc.groupID}).then(groupDoc => {
                    if (groupDoc) {
                        groupfeed.findOneAndUpdate({_id: req.body.pageID, favorite: {$nin: [req.user]}}, {$push: {'favorite': req.user}}).then(doc => {
                            ...
                            groupfeed.findOneAndUpdate({_id: req.body.pageID, favorite: {$in: [req.user]}}, {$pull: {'favorite': req.user}}).then(result => {
                ...
                groupfeed.findOneAndUpdate({_id: req.body.pageID}, {$addToSet: {'share': reciepent}}).then(() => {
                    groupfeed.findById(req.body.pageID).then(doc => {
                        if (doc) {
                            ...
                            sharecontent(groupfeed, feed, req.body.pageID, reciepent, req.user, req.username, req.userImage, shareInfo.groupID, groupDoc.title).then(shareInfo => {
                                for (let cnt of shareInfo) {
                                    notifications('groupFeedShare', cnt.userID, {userID: req.user, ID: cnt.pageID}, false);
                                }
                            });
                            return
                        }
                    ...
                    group.findOne({_id: groupID, member: {$in: [req.user]}}).then(groupDoc => {
                        if (groupDoc) {
                            ...
                            if (checked === reciepent.length) {
                                sharecontent(groupfeed, groupfeed, req.body.pageID, reciepent, req.user, req.username, req.userImage,
    ...
    if (req.header !== null && req.header('data-categ') === 'getOneAndDelete') {
        groupfeed.findOne({_id: req.body.pageID}).then(doc => {
            ...
            return Promise.reject('This group feed could not be deleted');
        }).catch(err => {
            res.status(500).send(err)
        })
        return
