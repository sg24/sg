...
const {grouppost, grouppostchat, tempFile, connectStatus} = require('../../serverDB/serverDB');
...
router.post('/', authenticate, (req, res, next) => {
        ...
        grouppost.findById(req.body.pageID).then(doc => {
            if (doc) {
                if (doc && doc.chat && doc.chat._id) {
                    grouppostchat.findOne({_id: objectID(doc.chat._id)}).then(result => {
                        let total = 0;
                        if (result) {
                            let chat = result.chat.filter(cnt => cnt.replyChat === false);
                            total = chat.length;
                        }
                        grouppostchat.aggregate([{
    ...
    if (req.header !== null && req.header('data-categ') === 'getReply') {
        grouppostchat.findById(req.body.cntID).then(doc => {
            if (doc) {
    ...
    if (req.header !== null && req.header('data-categ') === 'sendChat') {
        formInit(req, formidable).then(form => {
            ...
            savetemp(mediaList, 'grouppostchat', req.user).then(tempFileID => {
                uploadToBucket(mediaList, fields.description).then(media => {
                    ...
                    Promise.all([grouppostchat.findByIdAndUpdate(fields.cntID, {$push: {chat: cnt}}),
                        tempFile.findOneAndUpdate({userID: cnt.authorID, "tempFiles.id": tempFileID}, {$pull: {tempFiles: {id: tempFileID}}}),
                        grouppost.findOneAndUpdate({_id: fields.pageID, 'chat.user.authorID': {$ne: req.user}}, {$push: {'chat.user': {authorID: req.user, username: req.username, userImage: req.userImage}}})]).then(cnt => {
                        let total = cnt[0].chat.length + 1;
                        grouppost.findByIdAndUpdate(fields.pageID, {'chat.total': total}).then(result => {
                            res.status(200).send({_id, created, media, pageInfo: {_id: fields.pageID, chat: {total, user: result.chat.user, _id: result.chat._id}}})
                        })
                        
                        grouppostchat.findById(fields.cntID).then(doc => {
                            if (doc) {
                                let chat = doc.chat
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteChat') {
        Promise.all([deleteMedia(JSON.parse(req.body.media)),
            grouppostchat.findByIdAndUpdate(req.body.chatID, {$pull: {chat: {'_id': req.body.cntID}}})]).then(cnt => {
            grouppost.findByIdAndUpdate({_id: req.body.pageID}, {'chat.total': cnt[1].chat.length - 1}).then(result => {
    ...
    if (req.header !== null && req.header('data-categ') === 'deleteReply') {
        deleteMedia(JSON.parse(req.body.media)).then(() => {
            grouppostchat.findOne({_id: req.body.chatID}).then(doc => {
                if (doc) {
                    let chat = doc.chat;
                    ...
                    if (removeChatItem) {
                        ...
                        if (chatItem) {
                            ...
                            doc.updateOne({chat: removeChat}).then(() => {
                                grouppost.findByIdAndUpdate({_id: req.body.pageID}, {'chat.total': removeChat.length}).then(result => {
