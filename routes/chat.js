    ...
    if (!req.authType) {
        if (req.params.categ === 'group'){
            group.findOne({_id: mongoose.mongo.ObjectId(req.params.id), _isCompleted: true}).then(grpdet => {
                if (grpdet && grpdet.groupMode && grpdet.groupMode === 'general') {
                    group.findOne({_id: mongoose.mongo.ObjectId(req.params.id), _isCompleted: true, 
                        $or: [ { member: { $in: req.user } }, { authorID:  req.user } ]}).then(cnt => {
                        if (!cnt) {
                            group.findOneAndUpdate({_id: mongoose.mongo.ObjectId(req.params.id), _isCompleted: true}, {$addToSet: { member : req.user }, $inc: {'memberTotal': 1}}).then((grp) => {
                                if (grp) {
        ...
        res.cookie('redirect', `/chat/${req.params.categ}/${req.params.id}`);
        res.redirect('/login');
    }
})
