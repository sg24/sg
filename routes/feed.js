...
const {feed, advert, user, connectStatus} = require('../serverDB/serverDB');
...
router.post('/', authenticate, (req, res, next) => {
    ...
    if (req.header !== null && req.header('data-categ') === 'getFeed') {
        getshare(req, feed, 'feed').then(({updateResult, loadMore}) => {
        ...
        feed.findOneAndUpdate({_id: req.body.pageID}, {$addToSet: {'share': reciepent}}).then(() => {
            feed.findById(req.body.pageID).then(doc => {
                if (doc) {
                    ...
                    sharecontent(reciepent, 'feed', req.user, req.username, req.userImage, req.body.pageID).then(() => {
                        for (let userID of reciepent) {
                            notifications('feedShare', userID, {userID: req.user, ID: req.body.pageID}, false);
                        }
                    });
                    return
                }
    ...
    if (req.header && req.header('data-categ') === 'searchFeed') {
        getshare(req, feed, 'feed', {$text: {$search: req.body.searchCnt}}).then(({updateResult, loadMore}) => {
    ...
    if (req.header !== null && req.header('data-categ') === 'getOneAndDelete') {
        feed.findOne({_id: req.body.pageID, authorID: req.user}).then(doc => {
