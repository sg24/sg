...
router.post('/', authenticate, (req, res, next) => {
    if (req.header !== null && req.header('data-categ') === 'getonefeed') {
        feed.findOne({_id: req.body.cntID, authorID: req.user}).then(result => {
            if (result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'getByAuthor') {
        Promise.all([user.findById(req.body.searchCnt)]).then(doc => {
            feed.find({ _isCompleted: true, block: {$nin: [req.user]}, authorID: req.body.searchCnt, 'shareInfo.authorID': null})
                .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
                let updateResult = [];
                if (result) {
                    for (let cnt of result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'hashSearch') {
        feed.find({ _isCompleted: true, block: {$nin: [req.user]}, hashTag: {$in: [req.body.searchCnt]}, 'shareInfo.authorID': null})
            .skip(req.body.start).limit(req.body.limit).sort({_id: -1}).then(result => {
            let updateResult = [];
            if (result) {
                for (let cnt of result) {
    ...
    if (req.header !== null && req.header('data-categ') === 'setFavorite') {
        feed.findOneAndUpdate({_id: req.body.pageID, favorite: {$nin: [req.user]}}, {$push: {'favorite': req.user}}).then(doc => {
            if (doc) {
                return res.status(200).send({pageInfo: {_id: req.body.pageID, isFavored: true, favorite: doc.favorite.length + 1}});
            }
            feed.findOneAndUpdate({_id: req.body.pageID, favorite: {$in: [req.user]}}, {$pull: {'favorite': req.user}}).then(result => {
    ...
    if (req.header !== null && req.header('data-categ') === 'setShare') {
        ...
        feed.findOneAndUpdate({_id: req.body.pageID}, {$addToSet: {'share': reciepent}}).then(() => {
            feed.findById(req.body.pageID).then(doc => {
                if (doc) {
                    sharecontent(feed, feed, req.body.pageID, reciepent, req.user, req.username, req.userImage, 
                        ...
                        for (let cnt of shareInfo) {
                            notifications('feedShare', cnt.userID, {userID: req.user, ID: cnt.pageID}, false);
                        }
                    });
                    return
                }
                    ...
                    group.findOne({_id: groupID, member: {$in: [req.user]}}).then(groupDoc => {
                        if (groupDoc) {
                            ...
                            if (checked === reciepent.length) {
                                sharecontent(feed, groupfeed, req.body.pageID, reciepent, req.user, req.username, req.userImage,
    ...
    if (req.header && (req.header('data-categ') === 'searchFeed' || req.header('data-categ') === 'searchFeedTab')) {
        feed.find({_isCompleted: true, block: {$nin: [req.user]}, $text: {$search: req.body.searchCnt} })
            ...
            res.status(200).send({page: updateResult, loadMore: result.length > 0, tabPage: req.header('data-categ') === 'searchFeedTab'});
        }).catch(err => {
            res.status(500).send(err)
        })
        return
    ...
    if (req.header !== null && req.header('data-categ') === 'getOneAndDelete') {
        feed.findOne({_id: req.body.pageID}).then(doc => {
            ...
            return Promise.reject('This feed could not be deleted');
        }).catch(err => {
            res.status(500).send(err)
        })
        return
