...
class Report extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            inputHashTag: [],
            showUpload: false,
            reportDet: this.props.route.params
        }
    }
    ...
    componentDidMount() {
        if (!this.state.reportDet.navigationURI || !this.state.reportDet.page) {
    ...
    closePickerHandler = () => {
        this.setState({ showEmoji: false, showUpload: false})
    }

    submitHandler = () => {
        let cnt = {
            content: this.state.formElement.content.value,
            page: this.state.reportDet.page,
            pageID: this.state.reportDet.pageID,
            uploadFile: this.state.uploadFile,
            hashTag: this.state.inputHashTag
        }
        this.props.onSubmitForm(cnt, this.state.reportDet.cntType);
    }
    ...
    resetFormHandler = () => {
        ...
        this.setState({
            ...
            formIsValid: false, showEmoji: false,selection: {start: 0, end: 0},uploadFile: [],inputUri: [],inputHashTag: [], showUpload: false
        })
    }
...
const mapStateToProps = state => {
    return {
        submitError: state.addForm.pageReportSubmitError,
        submitted: state.addForm.pageReportSubmitted,
        start: state.addForm.pageReportStart,
...
const mapDispatchToProps = dispatch => {
    return {
        onSubmitForm: (formData, cntType) => dispatch(actions.submitAddFormInit(formData, cntType)),
