...
class Questions extends Component {
    ...
    componentDidMount() {
        ...
        //         this.props.onFetchPage(0, this.props.settings.page.fetchLimit, 'question', 'getQuestion', null, this.state.tabPage, this.props.fetchCnt[0]._id);
        //         return;
        //     }
        // });
        this._unsubscribeFocus = this.props.navigation.addListener('focus', () => {
            this.props.onFetchPage(0, this.props.settings.page.fetchLimit, 'question', 'getQuestion', null, this.state.tabPage);
            // if (!this.props.fetchCnt || (this.props.fetchCnt && this.props.fetchCnt.length < 1)) {
            //     this.props.onFetchPage(0, this.props.settings.page.fetchLimit, 'question', 'getQuestion', null, this.state.tabPage);
            // }
        });
        this._unsubscribeBlur = this.props.navigation.addListener('blur', () => {
            this.props.onPageReset('question');
            this.setState({ pageID: null, pageCntID: null, showSearch: false,search: '',showOption: false,showSettings: false, show: false, showSelectGroupPicker: null})
        });
    ...
    reloadFetchHandler = () => {
        if (this.state.search.trim().length > 0) {
            return this.props.onSearchCnt(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit, 'question', 'searchQuestion', this.state.search, this.state.tabPage);
        }
        this.props.onFetchPage(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit, 'question', 'getQuestion', null, this.state.tabPage);
    }
    ...
    searchPageHandler = (cnt) => {
        ...
        if (cnt && cnt.length > 0) {
            this.props.onSearchCnt(0, this.props.settings.page.fetchLimit, 'question', 'searchQuestion', cnt, this.state.tabPage);
        }
    }
    ...
    loadMoreHandler = () => {
        if (this.state.search.trim().length > 0) {
            return this.props.onSearchCnt(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit,
                 'question', 'searchQuestion', this.state.search, this.state.tabPage);
        }
        this.props.onFetchPage(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit, 'question', 'getQuestion', null, this.state.tabPage);
    }
    ...
    render() {
        ...
        if (this.props.fetchCnt && this.props.fetchCnt.length > 0 && this.state.show) {
...
const mapStateToProps = state => {
    return {
        ...
        fetchCnt: state.page.fetchQuestion ? state.page.fetchQuestion.filter(cnt => cnt.tabPage === TABPAGE) : null,
