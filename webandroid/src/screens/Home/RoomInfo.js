...
class Groups extends Component {
    ...
    componentDidMount() {
        if (this.state.groupID) {
            this._unsubscribe = this.props.navigation.addListener('focus', () => {
                if (this.state.groupID) {
                    this.props.onFetchPage(0, this.props.settings.page.fetchLimit, 'group', 'getGroupDet', this.state.groupID)
                } else {
    ...
    reloadFetchHandler = () => {
        this.props.onFetchPage(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit, 'group', 'getGroupDet', this.state.groupID);
    }
    ...
    enterGroupHandler = () => {
        this.props.navigation.navigate('GroupPreview', {pageID: this.state.groupID});   
    }
    ...
    cancelRequestHandler = () => {
        this.props.onPageReaction('group', this.state.groupID, 'cancelRequest');
    }
    ...
    loadMoreHandler = () => {
        ...
        this.props.onFetchPage(this.props.fetchCnt ? this.props.fetchCnt.length : 0, this.props.settings.page.fetchLimit, 'group', 'getGroupDet');
    }
    ...
    render() {
        ...
        if (this.props.fetchCnt && this.props.fetchCnt.length > 0) {
            let groupInfo = this.state.groupID ? this.props.fetchCnt.filter(cnt => cnt._id === this.state.groupID)[0] : null;
            let groupInfoMedia = groupInfo ? groupInfo.media.filter(cnt => cnt.bucket === 'image')[0] || {} : {};
            ...
            let startPageReaction = this.props.pageReaction.length > 0 ? 
                this.props.pageReaction.filter(id => id === groupInfo ? groupInfo._id : null).length > 0 ? true : false : false;
                cnt = (
                        ...
                        {groupInfo ?  (
                            ...
                            <View>
                                <ImageBackground 
                                    ...
                                    <Text style={[styles.textStyle, {fontWeight: 'bold'}]}>{ groupInfo.member } Members</Text>
                                    <View style={{justifyContent: 'flex-start', alignItems: 'flex-start', marginVertical: 10}}>
                                        <Button
                                            onPress={groupInfo.isMember ? this.enterGroupHandler : 
                                                groupInfo.isPending ? this.cancelRequestHandler : 
                                                groupInfo.isPendingApprove ? () =>  this.cancelExamHandler(groupInfo._id, null, 'cancelApprove', false, 'Cancelling this request will remove the exam you have written !'): 
                                                groupInfo.isPendingMark ? this.cancelExamHandler(groupInfo._id, null, 'cancelMark', false, 'Cancelling this request will remove the exam you have written !'): 
                                                () => this.requestHandler(groupInfo._id, groupInfo, groupInfo.isPublic, groupInfo.enableRule, groupInfo.enableCbt)}
                                            title={groupInfo.isMember ? 'Chat' : 
                                                groupInfo.isPending ? 'Cancel Request' : 
                                                !groupInfo.isPublic ? 'Request' :
                                                groupInfo.isPendingApprove ? 'Cancel Request' :
                                                groupInfo.isPendingMark ? 'Cancel Request' : 'Join'}
                                            style={groupInfo.isMember? styles.actionButton : styles.actionButtonAlt}
                                            textStyle={groupInfo.isMember ? styles.textStyle : styles.actionButtonText}
                                            disabled={startPageReaction && (groupInfo.isPending || !groupInfo.isMember )}/>
                                    </View>
                                </ImageBackground>
