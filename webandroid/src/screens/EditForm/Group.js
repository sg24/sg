...
class Group extends Component {
    ...
    componentDidUpdate() {
        if (!this.state.loaded && this.props.editFormCnt) {
            ...
            this.setState({
                duration: {hour: {value: hour,validation: {required: true,minLength: 1,maxNumber: 24,numbersOnly: true},valid: hour ? true : false,touched: false},
                ...
                content: {value: form.content,validation: {required: true,minLength: 1},valid: form.content ? true : false,touched: false,focus: false,inputHashTag: checkHashtag(form.content)},
                cbt: {value: form.enableCbt,validation: {},valid: true},rule: {value: form.enableRule,validation: {},valid: true},autoJoin: {value: form.autoJoin,validation: {},valid: true},
                passMark: {value: String(form.passMark),validation: {required: true,minLength: 1,maxNumber: 100,numbersOnly: true},valid: form.passMark ? true : false,touched: false},
                roomType: {value: form.roomType,validation: {},option:  [{title: 'Public', icon: {name: 'lock-open-outline', size: 18}},{title: 'Private', icon: {name: 'lock-closed-outline', size: 18}}],valid: form.roomType ? true : false,}},
                formIsValid: true,durationIsValid: duration !== 0,durationError: null,totalDuration: form.duration,
                uploadFile, loaded: true,fetchedUploadFile: uploadFile, fetchedQuestion: form.cbt ? form.cbt : {}
            });
        }
        ...
        if (this.state.resetAll) {
            this.setState({
                ...
                showUpload: false,examDetail: null,passMarkError: null, resetAll: false, loaded: false,fetchedUploadFile: [], fetchedQuestion: {}
            })
        }
    }
                ...
                <View style={styles.formWrapper}>
                    ...
                    <View style={styles.formElementWrapper}>
                        ...
                        <View style={styles.formElementButton}>
                            ...
                            <Button 
                                title={this.state.formElement.cbt.value ? 'Next' : 'Edit'}
                                style={styles.button}
...
const mapStateToProps = state => {
    return {
        editFormErr: state.editForm.fetchGroupError,
        editFormCnt: state.editForm.fetchGroup,
        submitError: state.editForm.groupSubmitError,
        submitted: state.editForm.groupSubmitted,
        start: state.editForm.groupStart,
