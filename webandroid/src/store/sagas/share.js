...
export function* shareInitSaga(action) {
    try {
        ...
        if (action.cntID === 'shareChat') {
            response = yield axios.post(`/${action.shareType}`,{
        ...
        for (let cnt of shareUpdates) {
            let updateResponse = yield axios.post(`/${cnt.shareType}`, {pageID: cnt.pageID, cnt: JSON.stringify(action.cnt), reciepent:  JSON.stringify(action.reciepent)},{
                headers: {
        ...
        let cnt = response && response.data  ? response.data : null;
        if (action.cntID === 'shareChat') {
